...
(function() {
    ...
    var People = function(Inherits, config) {
        ...
        this.me = function(cb) {
            this.createCall('GET', 'people/~:(' + fields.join(',') + ")", cb)(config);
        }

        this.url = function(url, cb) {
            this.createCall('GET', 'people/url=' + encodeURIComponent(url) + ":("+ short_fields.join(',') +")", cb)(config);
        };

        this.id = function(id, cb) {
            this.createCall('GET', 'people/id=' + id + ":("+ short_fields.join(',') +")", cb)(config);
        };
